...
function duitku_payment_notification()
{
    ...
    if (!empty($data)) {
        ...
        // Verify signature
        $merchantKey = $config['duitku_merchant_key'];
        ...
        if ($signature === $calculatedSignature) {
            ...
            if ($transaction) {
                ...
                if ($resultCode === '00' && $transaction['status'] != 2) {
                    if (Package::rechargeUser($user['id'], $transaction['routers'], $transaction['plan_id'], $transaction['gateway'], $data['paymentCode'])) {
                        $transaction->pg_paid_response = json_encode($data);
                        $transaction->payment_method = $data['paymentCode'];
                        $transaction->payment_channel = $data['productDetail'];
                        $transaction->paid_date = date('Y-m-d H:i:s');
                        $transaction->status = 2;
                        $transaction->save();
                        $msg = 'Payment successful and package activated.';
                    } else {
                        Message::sendTelegram("duitku_payment_notification: Activation FAILED: \n\n" . json_encode($data, JSON_PRETTY_PRINT));
                        $msg = 'Failed to activate package.';
                    }
                } elseif ($resultCode === '01') {
                    $transaction->pg_paid_response = json_encode($data);
                    $transaction->status = 3; // Mark transaction as failed
                    $transaction->save();
        ...
        die(json_encode(['status' => $resultCode, 'reference' => $reference, 'message' => $msg]));
    } else {
